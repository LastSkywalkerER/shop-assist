  ...
  return (
    <section>
      ...
      <div className="mx-4 glass rounded-2xl overflow-hidden border border-separator/15 shadow-[0_1px_4px_rgba(0,0,0,0.05)]">
        {rooms.map((room, i) => (
          <div key={room.id}>
            {i > 0 && <div className="h-px bg-separator/20 mx-4" />}
            <button
              ...
              className={`w-full text-left px-4 py-3 flex items-center justify-between transition-opacity ${
                room.id === roomId ? '' : 'active:opacity-70'
            ...
            >
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2">
                  ...
                  {room.is_personal && (
                    <span className="text-[11px] px-1.5 py-0.5 rounded-full bg-separator/20 text-text-hint shrink-0">
                      Personal
                    </span>
                  )}
      ...
      {currentRoom?.role === 'owner' && (
        <div className="mx-4 mt-3 glass rounded-2xl overflow-hidden border border-separator/15 shadow-[0_1px_4px_rgba(0,0,0,0.05)]">
          <div className="px-4 py-3">
            ...
            {!inviteCode ? (
              <button
                ...
                className="w-full py-2.5 rounded-xl bg-primary text-white text-[15px] font-medium active:opacity-80 transition-opacity disabled:opacity-50"
              >
                {isCreatingInvite ? 'Creating link...' : 'Share room'}
              </button>
            ) : (
              <div className="space-y-2">
                <div className="flex items-center gap-2 p-2 bg-bg-secondary/50 rounded-xl">
                  <span className="text-[12px] text-text-hint truncate flex-1">{webLink}</span>
                  <button
                    onClick={handleCopyLink}
                    className="shrink-0 px-3 py-1 rounded-lg bg-primary/10 text-primary-text text-[13px] font-medium active:bg-primary/20"
                  >
                    {copied ? 'Copied!' : 'Copy'}
                  </button>
                </div>
                <button
                  ...
                  className="w-full py-2.5 rounded-xl bg-[#2AABEE] text-white text-[15px] font-medium active:opacity-80 transition-opacity flex items-center justify-center gap-2"
                >
